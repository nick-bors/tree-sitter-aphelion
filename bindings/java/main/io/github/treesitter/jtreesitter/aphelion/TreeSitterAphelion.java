package io.github.treesitter.jtreesitter.aphelion;

import java.lang.foreign.*;

public final class TreeSitterAphelion {
    ...
    private static final TreeSitterAphelion INSTANCE = new TreeSitterAphelion();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterAphelion() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_aphelion");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-aphelion");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-aphelion");
